    ...
    items: [
      {
        ...
        bullets: [
          'Scratch Champion — Presented for all 3 categories (Regular, Senior, Super Senior). Winner is the player with the highest 2-day points total in the Championship Tournament.',
  ...
  return (
    <div>
      ...
      <div className="bg-forest rounded-lg px-6 py-5 text-center">
        ...
        <p className="text-gray-300 font-sans text-sm">Feel free to reach out to any board member or send us an email.</p>
      </div>
    </div>
  )
}
...
function EligibilityTab() {
  return (
    <div className="space-y-8">
      <EligSection title="Membership Requirement">
        <p>
      ...
      <EligSection title="Handicap Requirements">
      ...
      <EligSection title="Flight Assignments">
      ...
      <EligSection title="General Rules">
        <ul className="list-disc pl-5 space-y-2">
...
function EligSection({ title, children }) {
  return (
    <div>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {board.map((member) => {
          ...
          return (
            <div key={member.name} className="bg-white border border-gray-200 rounded-lg p-6 hover:border-gold hover:shadow-sm transition-colors">
              <div className="flex items-center gap-4 mb-4">
                {member.photo ? (
                  <img src={member.photo} alt={member.name} className="w-14 h-14 rounded-full object-cover border-2 border-gold" loading="lazy" />
                ) : (
                ...
                <div>
                  ...
                  <p className="text-gold text-xs font-sans font-semibold uppercase tracking-wide mt-0.5">{member.role}</p>
                </div>
              </div>
              <div className="text-gray-600 text-sm font-sans space-y-1">
                ...
                {member.since && <p className="text-gray-500 text-xs">Serving since {member.since}</p>}
              </div>
            </div>
          )
        })}
...
export default function Info() {
  useEffect(() => { document.title = 'Info | CGA 2026' }, [])
  const [tab, setTab] = useState('rules')

  return (
    <PageWrapper className={tab === 'rules' ? 'max-w-3xl' : undefined}>
      <div className="mb-8">
        <h1 className="section-title text-3xl sm:text-4xl">Association Info</h1>
